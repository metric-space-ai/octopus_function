...
### AI function section
device = config["device_map"]["device"]
...
def load_models():
    ...
    model_dict = {}
    model = AutoModelForSeq2SeqLM.from_pretrained(real_name, torch_dtype=torch.bfloat16).to(config["device_map"]["device"])
...
def translation(model_name, selection_mode, source, target, text):
    ...
    translator = pipeline('translation', model=model, tokenizer=tokenizer, src_lang=source_code, tgt_lang=target_code, device=config["device_map"]["device"])
